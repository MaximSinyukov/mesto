...
function escListener (evt, element) {
  if (evt.keyCode === 27) {
    closePopup(element)
  }
}

function clickEmpty (evt, element) {
...
const openPopup = (element) => {
  ...
  element.addEventListener('click', (evt) => {clickEmpty(evt, element)});
  addEventListener('keydown', (evt) => {escListener(evt, element)});
};

const closePopup = (element) => {
  element.classList.remove('popup_opened');
  element.removeEventListener('click', (evt) => {clickEmpty(evt, element)});
  removeEventListener('keydown', (evt) => {escListener(evt, element)});
};
